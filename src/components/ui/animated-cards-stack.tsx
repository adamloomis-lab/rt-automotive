...
// Recolored for the R/T dark performance theme (charcoal panels, red accent).
const cardVariants = cva('absolute will-change-transform', {
  variants: {
    variant: {
      dark: 'flex size-full flex-col items-center justify-center gap-6 rounded-2xl border border-line bg-pitch-3/90 p-8 backdrop-blur-md',
      light:
        'flex size-full flex-col items-center justify-center gap-6 rounded-2xl border border-line bg-pitch-2/90 p-8 backdrop-blur-md',
    },
  },
  defaultVariants: {
    variant: 'dark',
      ...
      <div className={cn('flex items-center gap-2', className)} ref={ref} {...props}>
        <div className="flex items-center">
          {[...Array(filledStars)].map((_, i) => (
            <svg key={`filled-${i}`} className="size-5 text-inherit" fill="currentColor" viewBox="0 0 20 20">
              <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.286 3.957a1 1 0 00.95.69h4.162c.969 0 1.371 1.24.588 1.81l-3.37 2.448a1 1 0 00-.364 1.118l1.287 3.957c.3.921-.755 1.688-1.54 1.118l-3.37-2.448a1 1 0 00-1.175 0l-3.37 2.448c-.784.57-1.838-.197-1.54-1.118l1.287-3.957a1 1 0 00-.364-1.118L2.05 9.384c-.783-.57-.38-1.81.588-1.81h4.162a1 1 0 00.95-.69l1.286-3.957z" />
            </svg>
          ))}
          {fractionalPart > 0 && (
            <svg className="size-5 text-inherit" fill="currentColor" viewBox="0 0 20 20">
              <defs>
                <linearGradient id="rt-half">
                  <stop offset={`${fractionalPart * 100}%`} stopColor="currentColor" />
                  <stop offset={`${fractionalPart * 100}%`} stopColor="rgb(75 78 82)" />
                </linearGradient>
              </defs>
              <path
          ...
          {[...Array(Math.max(0, emptyStars))].map((_, i) => (
            <svg key={`empty-${i}`} className="size-5 text-line" fill="currentColor" viewBox="0 0 20 20">
              <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.286 3.957a1 1 0 00.95.69h4.162c.969 0 1.371 1.24.588 1.81l-3.37 2.448a1 1 0 00-.364 1.118l1.287 3.957c.3.921-.755 1.688-1.54 1.118l-3.37-2.448a1 1 0 00-1.175 0l-3.37 2.448c-.784.57-1.838-.197-1.54-1.118l1.287-3.957a1 1 0 00-.364-1.118L2.05 9.384c-.783-.57-.38-1.81.588-1.81h4.162a1 1 0 00.95-.69l1.286-3.957z" />
            </svg>
          ))}
        </div>
